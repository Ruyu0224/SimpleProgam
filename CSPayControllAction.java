...
public class CSPayControllAction extends BaseAction  {
	...
	public String execute(RequestInfo requestInfo) {
       ...
       //冲销金额
        ...
        logger.info("获取明细表数量:"+detailTables.length);
		...
		for (Row costrow : costRows) {
			...
			for (Cell costcell : costCells) {
			 ...
			 logger.info("税序列:"+taxrate);
			 logger.info("税率:"+tax);
			 logger.info("税码:"+taxnum);
			for (Row recerow : receRows) {
				//获取收款明细单元数据
				logger.info("获取收款明细数据");
